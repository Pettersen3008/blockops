...
export function WorldsPage({ session }: { session: Session }) {
  ...
  const replace = useMutation({
    mutationFn: (selected: File) => api.replaceWorld(session.csrfToken, selected),
    onSuccess: () => {
      setFile(null);
  ...
  return (
    <>
      ...
      {replace.isError ? <Notice tone="danger">{errorMessage(replace.error)}</Notice> : null}
      ...
      <div className="two-column-grid">
        <Card className="world-card">
          ...
          <a className="button button--secondary" href="/api/v1/world/download"><Download aria-hidden="true" /> Prepare download</a>
        </Card>
        <Card className="world-card">
          ...
          {canReplace ? (
            <>
              ...
              <input ref={inputRef} className="sr-only" id="world-file" type="file" accept=".zip,application/zip" onChange={selectFile} />
              {file ? <div className="selected-file"><FileArchive aria-hidden="true" /><div><strong>{file.name}</strong><span>{formatBytes(file.size)}</span></div><Button variant="ghost" className="icon-button" aria-label="Remove selected file" onClick={() => setFile(null)}><X aria-hidden="true" /></Button></div> : null}
      ...
      <ConfirmDialog open={confirmOpen} title="Replace the current world?" description="BlockOps will stop the configured Minecraft container, move the existing worlds into a temporary rollback directory, install this ZIP, and start the server. Players will be disconnected." confirmLabel="Replace world" dangerous busy={replace.isPending} onClose={() => setConfirmOpen(false)} onConfirm={() => file && replace.mutate(file)} />
    </>
  );
}
