...
4. Run `make test`, `make build`, and the relevant browser workflow before opening a pull request. Deployment changes also run `make compose-config`; changes to the real Docker, RCON, or world paths run `make integration-up` and `make integration-down`.
