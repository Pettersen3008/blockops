...
4. Run `make test`, `make build`, and the relevant browser workflow before opening a pull request. Deployment changes also run `make compose-config`; changes to the real Docker, RCON, or world paths run `make integration-up`, `make test-integration`, and `make integration-down`. Run `make test-destructive` before `make integration-down` when lifecycle or world-replacement behavior changes.
5. Explain new dependencies and avoid client-side secrets, raw HTML sinks, or long-lived browser tokens.
6. Update the OpenAPI document and [security model](docs/SECURITY.md) when behavior or boundaries change.
