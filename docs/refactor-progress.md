...
### FE-26 — done — 2026-08-24
- Changed: deleted `application.css`, moved its shared primitive, shell, authentication, route-state, control, status, and responsive styling to the components that own it, and kept only imports, theme tokens, document defaults, reduced motion, and visible focus behavior in `globals.css`
- Simplest design: Tailwind utilities at existing call sites preserve the visual rules without a new styling dependency, component stylesheet, or utility abstraction; the reusable Input primitive alone keeps the common control baseline because it owns that element
- Tests: updated the Audit assertion from the removed implementation selector to the rendered status utilities; the full suite remains 28 files and 138 tests
- Verified: `pnpm --config.verify-deps-before-run=warn verify` green, 0 dependency-cruiser violations, Knip only reports its existing excluded-CSS configuration hint, production build completed, `git diff --check` green, and no `application.css`, legacy selector, or stale class call site remains
...
### E1 Bun workspace — done — 2026-08-27
- Changed: added the private root Bun 1.4.0 workspace for `frontend` and `packages/*`, a text `bun.lock` migrated from the pnpm graph, isolated linking, and Bun commands in package metadata, Make, Docker, CI, Dependabot, and current contributor instructions. Go remains outside the workspace.
- Dependency and lifecycle scripts: the migration preserves the pnpm-resolved package versions, including Rsbuild 2.1.13 and Vitest 4.1.10. `bun pm untrusted` found no blocked lifecycle scripts, so `trustedDependencies` stays absent. `bun audit --audit-level=high` found no high-or-higher findings across 472 packages, with five lower-severity findings.
- Baseline: macOS Darwin 25.6.0 arm64, Bun 1.4.0, Node 24.20.0, Go 1.25.0, pnpm 11.7.0. pnpm verification passed with 28 files and 138 tests in 89.93s at 321.5 MB peak resident memory, producing 1006.2 kB, 520.6 kB gzip. Disposable pnpm installs took 13.55s cold-cache and 9.10s warm-cache, at 485.1 MB and 430.7 MB peak resident memory.
- Bun: a disposable empty-cache frozen install took 5.65s at 185.2 MB peak resident memory; a disposable warm-cache frozen install took 0.95s at 12.0 MB. A second root `bun install --frozen-lockfile` made no `bun.lock` changes. `bun run --cwd frontend verify` passed typecheck, lint, 28 files and 138 tests, dependency-cruiser, Knip, and the unchanged Rsbuild build in 43.04s at 313.7 MB, producing 1006.2 kB, 520.5 kB gzip.
- Compatibility: Bun runs the existing Node-targeted TypeScript, ESLint, Vitest, dependency-cruiser, Knip, Rsbuild, and Playwright commands without a runtime override. Direct `/players` loads through a temporary Go binary with the current frontend build embedded.
- Verification: the production Go-server Playwright journey passed 1/1 in 29.2s after its direct-route check moved from the repurposed header to URL preservation and its uptime metric check became exact. Docker Compose validation passed; `docker build --tag blockops:e1 .` passed with the Bun image and confirmed runtime user `10001:10001`. The Linux container build provides the clean Linux Bun install proof; remote GitHub Actions remains unverified.
- Rollback: restore the E1-edited command files and `frontend/pnpm-lock.yaml`, then remove only the new root `package.json`, `bunfig.toml`, and `bun.lock`. No application source change is part of the rollback.
- Deleted: `frontend/pnpm-lock.yaml` and all active pnpm commands. Bun is the only package manager and lockfile.

### E2 Bun build and test toolchain — done — 2026-08-27
- Changed: `frontend/build.ts` and `frontend/dev.ts` replace Rsbuild's production bundle and development server, `bun:test` replaces Vitest, and `frontend/bunfig.toml` owns the test preload and the development Tailwind plugin. Package scripts, `tsconfig.node.json`, `knip.json`, and `.gitignore` follow. Docker, Make, CI, and Compose already ran Bun commands from E1 and needed no change.
- Deleted: `frontend/rsbuild.config.ts`, `frontend/vitest.config.ts`, `frontend/postcss.config.mjs`, `frontend/src/rsbuild-env.d.ts`, and the `@rsbuild/core`, `@rsbuild/plugin-react`, `vitest`, `jsdom`-era `postcss` and `@tailwindcss/postcss` dependencies. 406 packages install where E1 had 472.
- DOM: `bun:test` has no DOM, so `src/test/dom.ts` registers one. happy-dom deadlocked Testing Library's `waitFor` whenever a Base UI dialog closed while a request was in flight, driving Bun to 2.9 GB resident and a `formatting trait` panic with no test output. jsdom, the environment the suite was already written against, runs the same assertions cleanly. `configure({ asyncUtilTimeout: 5_000 })` covers Bun sharing one process across all 28 files.
- Assertions: every Vitest assertion survives. `vi.stubGlobal` became `Object.defineProperty`, `toHaveBeenCalledOnce` became `toHaveBeenCalledTimes(1)`, and the eight CSRF header checks kept `toBe("csrf-token")` with a widening cast, because `bun:test` types `toBe` against the narrowed variable where Vitest typed it `any`.
- Baseline: Rsbuild rebuilt from the same working tree produced 1006.3 kB / 520.6 kB gzip across 14 JS files, 1 CSS file and 12 woff2 files, and `pnpm verify` took 89.93s at FE-28.
- Bun: 1251.4 kB / 571.4 kB gzip across 18 JS files, 1 CSS file and 13 woff2 files. `bun run verify` takes 17s warm, runs 138 tests across 28 files with 508 assertions, and builds deterministically — two consecutive builds hash identically.
- Regression, explained: JS is 840.2 kB against 626.9 kB. About 175 kB of that is zod's locale bundle and JSON-Schema modules, which Rspack tree-shakes and Bun 1.4.0 keeps. Minimal repro: `import { z } from "zod"` plus one schema bundles to 283 kB under `bun build --minify` and still contains every non-English locale string. CSS is 105.9 kB against 75.9 kB raw but 16.4 kB against 16.4 kB gzipped.
- Compatibility findings: Bun 1.4.0 inlines every CSS `url()` as a data URL and ignores `loader: { ".woff2": "file" }` for them, against its own `docs/bundler/html-static.mdx`. Left alone this forced all 13 Fontsource subsets into the render-blocking stylesheet at +120 kB gzip; `build.ts` extracts them back into hashed files. Bun also does not define `process.env.NODE_ENV`, so the first working build shipped React's development build; `define` fixes it and accounts for 247 kB of JS.
- Security: the production build sets `sourcemap: "none"`. Go embeds and publicly serves every file under `dist`, and the Rsbuild config disabled production source maps for the same reason. The development WebSocket proxy now dials the API before completing the browser handshake, so a connection the API refuses fails the handshake instead of opening and closing with 1011.
- Verified: `bun install --frozen-lockfile` leaves `bun.lock` identical; `bun run --cwd frontend verify` green; Playwright 1/1 against the Bun development server and 1/1 against a Go binary with the Bun build embedded; all eight application routes plus an unknown path hard-refresh to the app shell through Go; hashed assets serve as `immutable` with correct content types and no source map exists; development proxy verified for HTTP, for a refused WebSocket upgrade against direct-to-Go behaviour, and for bidirectional frame forwarding including frames buffered before the client handshake; `docker build` green with runtime user `10001:10001`; `docker compose config --quiet` green; `git diff --check` green; no active Rsbuild, Vitest, PostCSS or pnpm reference remains.
- NOT verified: remote GitHub Actions, a Linux host outside the container build, Bun's React Compiler (not enabled — the brief gates it behind a clean plain build, and enabling it is not required to exit E2), `bun test --parallel` (the suite stays serial), and whether the zod tree-shaking gap has an upstream Bun issue.

### E3 UI package closeout — done — 2026-08-28
- Changed: audited `packages/ui` against E3 and closed concrete gaps. Link-styled navigation now keeps anchor semantics through `ButtonLink`; raw shared inputs now use `Input`; the Docker build copies the UI workspace; dependency-cruiser includes the UI package and rejects imports from it into `frontend`; global font declarations consume the package tokens.
- Simplest design: reused the existing variants, Input, Base UI `useRender`, and verification commands. No visual redesign, publishing pipeline, documentation app, dependency, state, cache, or feature logic change was added.
- Package boundary: removed the single-feature Avatar and Badge implementations from `packages/ui` and kept their markup in `player-row.tsx`. Deleted unused public exports for `cn`, `Label`, `NoticeTone`, and `StatusTone`. Deleted the stale shadcn `components.json`, whose aliases targeted removed frontend paths. The remaining root exports are explicit named exports used across product call sites or package-wide interaction rules.
- Accessibility: Base UI still owns dialog focus, keyboard, backdrop, and restore behavior. `ButtonLink` preserves native link semantics for two downloads and two router links. Existing tests cover accessible dialog names, disabled pending controls, destructive styling, and focus restore. The Base UI native-button warnings from the moved links are gone.
- Tokens: `packages/ui/src/styles/tokens.css` is the only color, radius, shadow, and font token source. `frontend/src/app/styles/globals.css` imports it once and contains only global element behavior after the imports.
- New files: none.
- State/cache: unchanged.
- Tests: no new test file. The existing 138 tests exercise the moved primitives through their product call sites, including focus, disabled, destructive, form, table, status, and link behavior.
