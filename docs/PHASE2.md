...
4. Rebuild `users` without `role` and with `fleet_owner`. SQLite cannot drop a column a `CHECK` constraint mentions, so this is the twelve-step table rebuild: create the new table, copy, drop the old, rename, recreate indexes. It runs inside the migration's transaction with `foreign_keys` deferred for the swap.
